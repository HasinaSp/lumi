  ...
  return (
    <main className="min-h-screen bg-[#f8f7f3] text-neutral-950 transition-colors dark:bg-black dark:text-white">
      ...
      >
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-3">
            {t.landing.methodSteps.map((step, index) => (
              <div className="rounded-3xl border border-white/50 p-8" key={step.title}>
                <span className="text-neutral-500 bg-emerald-500 rounded-full px-3 py-1">{index + 1}</span>
                <h3 className="mt-6 text-2xl font-medium">{step.title}</h3>
                <p className="mt-4 text-neutral-400">{step.description}</p>
              </div>
            ))}
          </div> 
        </div>
      ...
      <section id="offres" className="mx-auto max-w-7xl px-6 py-24">
        <p className="text-sm uppercase tracking-[0.35em] text-neutral-500 dark:text-neutral-400">
          {t.landing.offersEyebrow}
        </p>

        <h2 className="mt-4 max-w-3xl text-4xl font-semibold md:text-5xl">
          {t.landing.offersTitle}
        </h2>
